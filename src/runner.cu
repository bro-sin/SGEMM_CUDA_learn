...
void runMySgemm2DBlocktiling(int M, int N, int K, float alpha, float *A,
                             float *B, float beta, float *C) {
  // const uint BM = 128;
  // const uint BN = 128;
  // const uint BK = 8;
...
void runMySgemmVectorize(int M, int N, int K, float alpha, float *A, float *B,
                       float beta, float *C) {
  const uint BK = 8;
  const uint TM = 8;
  const uint TN = 8;
  if (M >= 128 and N >= 128) {
    ...
    dim3 gridDim(CEIL_DIV(N, BN), CEIL_DIV(M, BM));
    dim3 blockDim((BM * BN) / (TM * TN));
    mysgemmVectorize<BM, BN, BK, TM, TN>
        <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  } else {
    // this is a hacky solution to the underlying problem
    // of not having proper bounds checking in the kernel
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  case 15:
    runMySgemm2DBlocktiling(M, N, K, alpha, A, B, beta, C);
    break;
  case 16:
